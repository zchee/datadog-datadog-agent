...
func TestFIPSCiphersSuite(t *testing.T) {
	e2e.Run(t, &fipsServerSuite{}, e2e.WithProvisioner(awsdocker.Provisioner()), e2e.WithDevMode())
}
	...
	v.UpdateEnv(
		awsdocker.Provisioner(
			awsdocker.WithAgentOptions(
				dockeragentparams.WithFullImagePath("datadog/agent-dev:nicolas-guerguadj-microsoft-fork-test-py3"),
...
func (v *fipsServerSuite) TestFIPSCiphersTLSVersion() {
	...
	_, err := v.Env().RemoteHost.Execute("docker run --rm -d --network fips-network --name fips-server ghcr.io/datadog/apps-fips-server:main " + "rsa --tls-max=1.1")
