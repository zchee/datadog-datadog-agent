...
func NewTracer(config *config.Config, _ telemetryComponent.Component) (Tracer, error) {
	...
	connCloseEventHandler, err := initClosedConnEventHandler(config, closedCallback, connPool, connHasher, extractor)
	if err != nil {
		return nil, err
	}
...
func initClosedConnEventHandler(config *config.Config, closedCallback func(*network.ConnectionStats), connGetter ddsync.PoolGetter[network.ConnectionStats], connHasher *cookieHasher, extractor *batchExtractor) (*perf.EventHandler, error) {
