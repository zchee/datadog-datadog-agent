...
static __always_inline bool is_sql_command(const char *buf, __u32 start_offset, __u32 buf_size) {
    if (buf_size < start_offset + SQL_COMMAND_MAX_SIZE) {
        return false;
    }
...
#pragma unroll (SQL_COMMAND_MAX_SIZE)
    for (int i = 0; i < SQL_COMMAND_MAX_SIZE; i++) {
        tmp[i] = buf[start_offset + i] - TO_CAPITAL * ((int)('a' <= buf[start_offset + i] && buf[start_offset + i] <= 'z'));
    }
