...
int BPF_TP_SYSCALL_ENTER(tracepoint__syscalls__sys_enter_open, const char *filename) {
    do_sys_open_helper_enter(filename);
    return 0;
}

SEC("tracepoint/syscalls/sys_exit_open")
int BPF_TP_SYSCALL_EXIT(tracepoint__syscalls__sys_exit_open, long ret) {
...
int BPF_TP_SYSCALL_EXIT(tracepoint__syscalls__sys_exit_openat, long ret) {
    do_sys_open_helper_exit(ctx, ret);
    return 0;
}
...
int BPF_TP_SYSCALL_EXIT(tracepoint__syscalls__sys_exit_openat2, long ret) {
    do_sys_open_helper_exit(ctx, ret);
    return 0;
}
