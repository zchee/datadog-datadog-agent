...
func (suite *LauncherTestSuite) TestSentLogExceedsTotalUsage() {
	...
	for _, filename := range files {
		file, err := os.Create(filepath.Join(suite.s.runPath, filename))
		require.NoErrorf(suite.T(), err, "Could not create file %q", filename)
		_, err = file.Write(dataOneMB)
		require.NoErrorf(suite.T(), err, "Could not write to file %q", filename)
		err = file.Close()
		assert.NoErrorf(suite.T(), err, "Could not close file %q", filename)
	}
	...
	for _, filename := range files {
		...
		require.NoErrorf(suite.T(), err, "Could not stat file %q", filename)
