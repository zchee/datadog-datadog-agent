...
func TestBatchStrategySendsPayloadWhenBufferIsFull(t *testing.T) {
	...
	s := NewBatchStrategy(input, output, flushChan, false, LineSerializer, 100*time.Millisecond, 2, 2, "test", &identityContentType{})
	s.Start()
...
func TestBatchStrategySendsPayloadWhenBufferIsOutdated(t *testing.T) {
	...
	clk := clock.NewMock()
	s := newBatchStrategyWithClock(input, output, flushChan, false, LineSerializer, timerInterval, 100, 100, "test", clk, &identityContentType{})
	s.Start()
...
func TestBatchStrategySendsPayloadWhenClosingInput(t *testing.T) {
	...
	clk := clock.NewMock()
	s := newBatchStrategyWithClock(input, output, flushChan, false, LineSerializer, 100*time.Millisecond, 2, 2, "test", clk, &identityContentType{})
	s.Start()
...
func TestBatchStrategySynchronousFlush(t *testing.T) {
	...
	strategy := NewBatchStrategy(input, output, flushChan, false, LineSerializer, time.Hour, 100, 100, "test", &identityContentType{})
	strategy.Start()
