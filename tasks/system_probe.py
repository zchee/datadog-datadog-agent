...
CLANG_VERSION_SYSTEM_PREFIX = "20.0"
...
@task
def check_for_misaligned_stack_objects(ctx: Context, build_dir: str | Path = None, use_debug_files: bool = False):
    if not build_dir:
    ...
    with ctx.cd("./tools/ebpf/stack-align"):
        ctx.run(f"go run . -build-dir {build_dir} {'-use-debug-files' if use_debug_files else ''}")
...
def object_files(ctx, kernel_release=None, with_unit_test=False, arch: str = CURRENT_ARCH, strip=False):
    build_object_files(
        ctx, kernel_release=kernel_release, with_unit_test=with_unit_test, arch=arch, strip_object_files=strip
    )
